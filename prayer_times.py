...
COUNTRY = "Egypt"
METHOD = 5
CACHE_FILE = "/tmp/waybar_prayer_times_cairo_extended.json"
OFFSET_FILE = "/tmp/waybar_prayer_offset.txt"
...
# --- معالجة التمرير بالماوس (Scroll) ---
# إذا تم استدعاء السكربت مع تمرير argument (up, down, reset)
if len(sys.argv) > 1:
    cmd = sys.argv[1]
    try:
    ...
    # إرسال إشارة لـ Waybar لتحديث الموديل فوراً
...
def build_cache():
    all_data = []
    now = datetime.now()
    # شهر سابق (-1)، شهر حالي (0)، وشهرين قادمين (1, 2)
    for mo in [-1, 0, 1, 2]:
    ...
    for day in all_data:
        ...
        cache_dict[date_str] = timings
        
    try:
        with open(CACHE_FILE, 'w') as f:
            json.dump({"cached_month": now.month, "data": cache_dict}, f)
    except Exception:
        pass
    return cache_dict
...
def main():
    ...
    if offset == 0:
        ...
        if not next_prayer_name:
            ...
            if tomorrow_str in times_db:
                next_prayer_time_str = times_db[tomorrow_str]["Fajr"]
    ...
    # إذا كان اليوم المعروض هو (يوم آخر) لا نعرض الوقت المتبقي
    else:
        tooltip = f"<span size='large' weight='bold'>أوقات الصلاة (القاهرة)</span>\n<span size='small' color='#a6adc8'>التاريخ: {target_str}</span>\n\n"
        for en_name, p_time in sorted_prayers:
            ar_name = PRAYER_NAMES[en_name]
            tooltip += f"{ar_name}: {p_time}\n"
            
        # إضافة نص يوضح أنه يمكن الضغط للعودة لليوم
        tooltip += "\n<span size='small' color='#f9e2af'><i>* اضغط بالزر الأوسط للعودة لليوم</i></span>"
        
        # تغيير شكل الأيقونة لتوضيح أننا نتصفح أيام أخرى
        text_out = f"📅 {target_date.strftime('%d/%m')}"
